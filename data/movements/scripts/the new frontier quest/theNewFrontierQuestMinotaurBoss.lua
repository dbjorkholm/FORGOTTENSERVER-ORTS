...
function onStepIn(cid, item, position, fromPosition)
	...
	if(item.actionid == 12135) then
		if(player:getStorageValue(Storage.TheNewFrontier.Questline) == 18) then
			addEvent(function(cid) local player = Player(cid) if not player then return true end if player:getStorageValue(Storage.TheNewFrontier.Questline) == 19 then player:teleportTo({x = 33145, y = 31419, z = 7}) player:say("You have passed the test. Report to Curos.", TALKTYPE_MONSTER_SAY) end end, 2 * 60 * 1000, cid) -- 2 minutes
	...
	elseif(item.actionid == 12136) then
		...
		player:teleportTo(destination)
		destination:sendMagicEffect(CONST_ME_TELEPORT)
		player:say("You left the area. Ask Curos again for the mission!", TALKTYPE_MONSTER_SAY)
	end
	return true
end
