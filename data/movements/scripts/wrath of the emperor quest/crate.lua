function catchPlayer(cid)
	...
	doCreatureSay(cid, "The guards have spotted you. You were forcibly dragged into a small cell. It looks like you need to build another disguise.", TALKTYPE_MONSTER_SAY)
	return true
end
...
function onStepIn(cid, item, position, fromPosition)
	if(isPlayer(cid)) then
		if(item.actionid == 8015) then
			doCreatureSay(cid, "You hear guards moving behind doors in the distance. If you have any sort of disguise with you, this is the moment to use it.", TALKTYPE_MONSTER_SAY)
		...
		elseif(item.actionid == 8021) then
			doCreatureSay(cid, "Guards heavily patrol this area. Try to stay hidden and do not draw any attention to yourself by trying to attack.", TALKTYPE_MONSTER_SAY)
