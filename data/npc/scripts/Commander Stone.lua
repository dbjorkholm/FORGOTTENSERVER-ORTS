...
local function creatureSayCallback(cid, type, msg)
	...
	if(msgcontains(msg, "mission")) then
		...
		npcHandler:say({
				"For your rank there are four missions avaliable {crystal keeper}, {spark hunting} ...", "If you lose a mission item you can probably buy it from Gnomally."
			}, cid)
	...
	elseif(msgcontains(msg, "spark")) then
		if player:getStorageValue(Storage.BigfootBurden.MissionRaidersOfTheLostSpark) < 1 and player:getStorageValue(Storage.BigfootBurden.RaidersOfTheLostSparkTimeout) < os.time() then
			...
			npcHandler.topic[cid] = 0
		elseif(npcHandler.topic[cid] == 1 or npcHandler.topic[cid] == 2) then
			if player:getStorageValue(Storage.BigfootBurden.ExtractedCount) == 7 and player:removeItem(18213, 1) then
				player:setStorageValue(Storage.BigfootBurden.Rank, player:getStorageValue(Storage.BigfootBurden.Rank) + 5)
				player:addItem(18422, 1)
				player:addItem(18215, 1)
	...
	elseif(msgcontains(msg, "extermination")) then
		if player:getStorageValue(Storage.BigfootBurden.MissionExterminators) < 1 and player:getStorageValue(Storage.BigfootBurden.ExterminatorsTimeout) < os.time() and player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 17 then
	...
	elseif(msgcontains(msg, "digging")) then
		if player:getStorageValue(Storage.BigfootBurden.MissionMushroomDigger) < 1 and player:getStorageValue(Storage.BigfootBurden.MushroomDiggerTimeout) < os.time() and player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 17 then
			npcHandler:say({
	...
	elseif(msgcontains(msg, "report")) then
		...
		elseif player:getStorageValue(Storage.BigfootBurden.QuestLine) >= 17 then
			...
			if player:getStorageValue(Storage.BigfootBurden.Rank) >= 120 then
				...
				npcHandler:say("Congratulations , you have increased your reputation to " .. player:getStorageValue(Storage.BigfootBurden.Rank) .. " points. By the way, you {rank} now allows you to take aditional missions from {Gnomeral} in {Gnomebase Alpha}. ", cid)
				return true
			end
		end
	end
