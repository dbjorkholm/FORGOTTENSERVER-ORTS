...
local function creatureSayCallback(cid, type, msg)
	...
	if(msgcontains(msg, "looking")) then
		if player:getStorageValue(900) == 11 then
			npcHandler:say("I'm the gnomish musical supervisor!", cid)
			npcHandler.topic[cid] = 1
		elseif player:getStorageValue(900) == 13 then
			npcHandler:say("I'm the gnomish musical supervisor!", cid)
	...
	elseif(msgcontains(msg, "musical")) then
		...
		elseif(npcHandler.topic[cid] == 2) then
			...
			player:setStorageValue(900, 14)
			player:setStorageValue(921, 0)
			npcHandler.topic[cid] = 0
		end
	end
	return true
