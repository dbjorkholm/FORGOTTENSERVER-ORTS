...
function creatureSayCallback(cid, type, msg)
	...
	if not npcHandler:isFocused(cid) then
		return false
	elseif msgcontains(msg, "trouble") and npcHandler.topic[cid] ~= 3 and Player(cid):getStorageValue(storage) < 1 then
	...
	elseif msgcontains(msg, "trouble will arise in the near future") and npcHandler.topic[cid] == 3 then
		...
		if player:getStorageValue(storage) < 1 then
			...
			player:setStorageValue(12111, player:getStorageValue(12111) + 1) -- The Inquisition Questlog- "Mission 1: Interrogation"
