...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "mission") then
		...
		elseif player:getStorageValue(Storage.ChildrenoftheRevolution.Questline) == 21 and player:getStorageValue(1060) < 1 then
			npcHandler:say(
						{
		...
		elseif player:getStorageValue(1060) == 11 then
			npcHandler:say("You - azzembled zze zzeptre? Hand it out, give it to me, will you? ", cid)
			npcHandler.topic[cid] = 19
		elseif player:getStorageValue(1060) == 12 then
			npcHandler:say(
						{
			...
			player:setStorageValue(1060, 13)
			npcHandler.topic[cid] = 0
		-- WRATH OF THE EMPEROR QUEST	
		end
	...
	elseif msgcontains(msg, "yes") then
		...
		elseif npcHandler.topic[cid] == 19 then
			if getPlayerItemCount(cid, 12327) >= 1 then
				...
				player:setStorageValue(1060, 12)
