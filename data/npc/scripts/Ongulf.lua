...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "project") and player:getStorageValue(Storage.TheNewFrontier.Questline) < 1 then
		if npcHandler.topic[cid] == 0 then
			npcHandler:say({"Well, it's a long story but you really should listen to understand what is going on here. You can also hear a short version of the story, but then don't blame me if you mess something up due to your undwarfish impatience. ...",
							"So what would you like to hear, the {long} story or the {short} version?"}, cid)
			npcHandler.topic[cid] = 1
		end
	elseif msgcontains(msg, "long") then
		if npcHandler.topic[cid] == 1 then
			npcHandler:say({"After centuries the resources beneath and around the Big Old One became alarmingly short. Some decades ago, the imperial mining guild financed a project to search and establish new mines far away from Kazordoon. ...",
							...
							"We would need help and additional resources. That's were you come into play. If you are interested, let's talk about possible missions."}, cid)
			npcHandler.topic[cid] = 2
		end
	elseif msgcontains(msg, "short") then
		if npcHandler.topic[cid] == 1 then
			npcHandler:say({"<grumbles> Well, we have found this place here full of promising resources and plan to create a new mining outpost. Of course this takes a lot of effort and organisation. ...",
							...
							"Do not explore any further though. You never know whom you might be messing with."}, cid)
	...
	elseif msgcontains(msg, "mission") then
		if player:getStorageValue(Storage.TheNewFrontier.Questline) < 1 and npcHandler.topic[cid] == 2 then
			npcHandler:say({"Listen, I can handle the organisation down here and my boys will handle the construction of the base fine enough. Actually, all you do down here is to stand in the workers' way. ...",
							"But there might be something for you to do outside the base. We need to learn more about the land up there. Take the lift and do some exploring. Find a passage leading out of the mountains. ...",
							"Do not explore any further though. You never know whom you might be messing with."}, cid)
			player:setStorageValue(Storage.TheNewFrontier.Questline, 1)
			player:setStorageValue(Storage.TheNewFrontier.Mission01, 1) --Questlog, The New Frontier Quest "Mission 01: New Land"
		...
		elseif player:getStorageValue(Storage.TheNewFrontier.Questline) == 3 then
			npcHandler:say({"Things are running fine so far. Actually so fine that we are short of supplies and men. I'd send a letter home but I guess sending you to get some assistance gives the whole affair a bit more urgency. ...",
		...
		elseif player:getStorageValue(Storage.TheNewFrontier.Questline) == 11 then
			npcHandler:say({"It seems things went from bad to worse! First we had some problems with the mine shafts we were building, and now that we found some precious veins in one of the new mines, and it happens to be the holiday resort of some hostile stone creatures! ...",
							...
							"So I reserved you the privilege to slay the leader! Use the mining lift to reach mine A07. The more stone creatures you kill, the better. Your mission, however, is to slay their leader, most likely some special stone beast."}, cid)
		...
		elseif player:getStorageValue(Storage.TheNewFrontier.Questline) == 14 then
			npcHandler:say({"Things are getting more and more complicated. You need to convince our friends that some intervention for their part is needed. ...",
							...
							"Well, I hope you understand the importance of this mission and got what it takes to fulfil it. So hurry up and get us the needed support."}, cid)
		...
		elseif player:getStorageValue(Storage.TheNewFrontier.Questline) == 20 then
			npcHandler:say({"Oh my. What a mess you have gotten yourself into. Well, at least you made it out alive. Whatever the value of a minotaur's promise might be, I guess that is the best we can get. ...",
							"Of course all those revelations lead to new problems and a new mission for you."}, cid)
		...
		elseif player:getStorageValue(Storage.TheNewFrontier.Questline) == 21 then
			npcHandler:say({"Ooook. Now that we managed to keep those orcs and minotaurs at bay at least for a while, we learn that the real meanies over here are some lizardmen. Just great, isn't it?. ...",
							...
							"If the orcs are right, they are somewhere in or behind those mountains in the north. I doubt you can reason with them in any way, but you'll have to try for the sake of Farmine."}, cid)
		...
		elseif player:getStorageValue(Storage.TheNewFrontier.Questline) == 27 then
			npcHandler:say({"Oh, my! That sounds like a real mess. For now this dragon empire seems otherwise engaged, but we will be on guard thanks to you my friend. We will continue to fortify the base. ...",
							...
							"Oh and one last thing: We convinced a carpet pilot to join us here. You'll find him on top of our lift in the mountains. I think he can offer you fast access to some cities back home."}, cid)
