...
local function creatureSayCallback(cid, type, msg)
	...
	if not npcHandler:isFocused(cid) then
		return false
	elseif(msgcontains(msg, "project") and player:getStorageValue(TheNewFrontier.Questline) < 1) then
	...
	elseif(msgcontains(msg, "mission")) then
		if(player:getStorageValue(TheNewFrontier.Questline) < 1 and npcHandler.topic[cid] == 2) then
			npcHandler:say(
						{
							"Listen, I can handle the organisation down here and my boys will handle the construction of the base fine enough. Actually, all you do down here is to stand in the workers' way. ...",
							"But there might be something for you to do outside the base. We need to learn more about the land up there. Take the lift and do some exploring. Find a passage leading out of the mountains. ...",
		...
		elseif(player:getStorageValue(TheNewFrontier.Questline) == 7) then
			...
			npcHandler.topic[cid] = 0	
		elseif(player:getStorageValue(TheNewFrontier.Questline) == 8) then
			npcHandler:say(
						{
		...
		elseif(player:getStorageValue(TheNewFrontier.Questline) == 10) then
			...
			npcHandler.topic[cid] = 0	
		elseif(player:getStorageValue(TheNewFrontier.Questline) == 11) then
			npcHandler:say(
						{	
		...
		elseif(player:getStorageValue(TheNewFrontier.Questline) == 15) then
			if(player:getStorageValue(TheNewFrontier.BribeKing) == 1 and player:getStorageValue(TheNewFrontier.BribeLeeland) == 1 and player:getStorageValue(TheNewFrontier.BribeAngus) == 1 and player:getStorageValue(TheNewFrontier.BribeWydrin) == 1 and player:getStorageValue(TheNewFrontier.BribeTelas) == 1 and player:getStorageValue(TheNewFrontier.BribeHumgolf) == 1) then
		...
		elseif(player:getStorageValue(TheNewFrontier.Questline) == 16) then
			npcHandler:say(
						{
		...
		elseif(player:getStorageValue(TheNewFrontier.Questline) == 20) then
			...
			npcHandler.topic[cid] = 0
		elseif(player:getStorageValue(TheNewFrontier.Questline) == 21) then
			npcHandler:say(
						{
		...
		elseif(player:getStorageValue(TheNewFrontier.Questline) == 27) then
			...
			player:setStorageValue(TheNewFrontier.Mission10, 1) --Questlog, The New Frontier Quest "Mission 10: New Horizons"
			npcHandler.topic[cid] = 0
		end
	end
	return true
