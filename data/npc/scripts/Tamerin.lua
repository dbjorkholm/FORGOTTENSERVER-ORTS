...
local function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, "animal cure")) then
		if(player:getStorageValue(Storage.InServiceofYalahar.Questline) == 30 and getPlayerItemCount(cid, 9734) >= 1) then
	...
	elseif(msgcontains(msg, "morik")) then
		if(player:getStorageValue(Storage.InServiceofYalahar.Questline) == 31 and getPlayerItemCount(cid, 9735) >= 1) then
			player:removeItem(9735, 1)
			player:setStorageValue(Storage.InServiceofYalahar.Questline, 32)
