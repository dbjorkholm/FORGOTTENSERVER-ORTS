...
function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "eremo") then
		...
		npcHandler.topic[cid] = 0
	elseif msgcontains(msg, "fan club membership") and player:getStorageValue(100000) == 15 and doPlayerTakeItem(cid, 10308, 1) then
	   	npcHandler:say({"Ah, {A fan club premium membership card}! You must be that intelligent fellow who wrote me all those flattering letters! Nice to finally meet my greatest admirer in person. Here, take this little token of appreciation. ...",
						"And now if you will excuse me, I have to attend urgent affairs of state."},cid, 4000)
	...
	elseif(msgcontains(msg, "flatter")) then
		if(npcHandler.topic[cid] == 2) then
			if(player:getStorageValue(1022) < 1) then
