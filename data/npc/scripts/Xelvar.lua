...
local function creatureSayCallback(cid, type, msg)
	...
	if not npcHandler:isFocused(cid) then
		return false
	elseif msgcontains(msg, "adventures") then
	...
	elseif msgcontains(msg, "gnomes") then
		if npcHandler.topic[cid] == 3 then
			npcHandler:say({"The gnomes preferred to keep our alliance and their whole {existence} a secret. They are a bit distrustful of others. ...",
							"They are quite self-sufficient and the fact that they are actually accepting some help is more than alarming. The gnomes are in real trouble and I am kind of an ambassador to find some people willing to {help}."}, cid)
			npcHandler.topic[cid] = 4
		end
	elseif msgcontains(msg, "help") then
		if npcHandler.topic[cid] == 4 then
			npcHandler:say({"The gnomes are locked in a war with an enemy that thins out their resources but foremost their manpower. We have suggested that people like you could be just the specialists they are looking for. ...",
							"If you are interested to {join} the gnomish cause I can arrange a meeting with their recruiter."}, cid)
			npcHandler.topic[cid] = 5
		end
	elseif msgcontains(msg, "join") then
		if npcHandler.topic[cid] == 5 then
			npcHandler:say({"I am glad to hear that. In the spirit of our own foreign legion we suggested the gnomes might hire heroes like you to build some kind of troop. They gave me that strange crystal to allow people passage to their realm. ...",
							...
							"Good luck to you and don't embarrass your race down there! Keep in mind that you are a representative of the big people."}, cid)
