...
local function creatureSayCallback(cid, type, msg)
	...
	if(msgcontains(msg, "mission")) then
		if player:getStorageValue(Storage.wrathOfTheEmperorQuest) == 32 and player:getStorageValue(1090) == 5 then
		...
		elseif player:getStorageValue(Storage.wrathOfTheEmperorQuest) == 34 then
			...
			player:setStorageValue(Storage.wrathOfTheEmperorQuest, 35)
			npcHandler.topic[cid] = 0
		end
	elseif(msgcontains(msg, "mission")) then
		if(npcHandler.topic[cid] == 1) then
			player:setStorageValue(Storage.wrathOfTheEmperorQuest, 33)
