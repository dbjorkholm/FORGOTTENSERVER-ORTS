...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "mission") then
		...
		elseif player:getStorageValue(9994) == 1 and player:getStorageValue(9995) < 1 then
			...
			npcHandler:say({"I need my heart back. I know where they have hidden it. Too afraid to destroy my beating heart, hah! ...", "It is in a dusty amphora in a sealed mass grave in the downmost cellar of my ancient home. The ruins lie to the north at the beach. ...", "Crash the amphoras to find my heart, and bring it to me."}, cid, 0, 1, 5000)
		elseif player:getStorageValue(9996) == 1 and player:getStorageValue(9997) < 1 then
			npcHandler:say({"Yes? You have it? You what? Not in the amphoras? You picked it off someone else?!? ...", "Well, the important thing is that you have it. Let's see if it's still in good shape. {Give} it to me."}, cid, 0, 1, 5000)
			npcHandler.topic[cid] = 4
		elseif player:getStorageValue(9997) == 1 and player:getStorageValue(9998) < 1 then
			player:setStorageValue(9998, 1)
			npcHandler:say({"I will need brains - don't laugh! Ahem. I will need a stimulated brain, to be precise. ...", "Use two half-eaten brains with the Brain Heater Machine in the Necromancer halls and bring me the fused, stimulated brain. Now go!"}, cid, 0, 1, 5000)
		...
		elseif player:getStorageValue(9950) == 1 and player:getStorageValue(9951) < 1 then
			...
			npcHandler:say({"Now that you have shown you've got the brains, I need you to show initiative. ...", "I will need something that can be adequately used as intestines. Something alive. Stuff it into this storage flask and return it to me!"}, cid, 0, 1, 5000)
		...
		elseif player:getStorageValue(9956) == 1 and player:getStorageValue(9957) < 1 then
			npcHandler:say({"I expected no less. Next is the Bonemarrow Altar, where the Dark Lord feasts upon the hallowed bones of...err. ...", "Ahem. Just take a yellowed bone or big bone, and hallow it - you don't know how? ...", "Huh. To HALLOW a yellowed bone, use it with one of the hallowed bonepiles in the Gardens Of Night. There are always bones around there - or were, in my days. ...", "Then, place the hallowed bone on the firebasin of the Bonemarrow Altar so the Dark Lord can consume it and grants us his power. Return to me after that."}, cid, 0, 1, 5000)
			player:setStorageValue(9957, 1)
		elseif player:getStorageValue(9957) == 1 and player:getStorageValue(9958) < 1 then
			npcHandler:say({"Are you still here? Come on, let's not laze about, go to the Gardens of Night - where? ...", "East of the Necromancer Halls, you can't miss it, just look for a lot of dark and white sand, dried trees, and priestesses! ...", "Or got a {problem} with finding bones? Hah!"}, cid, 0, 1, 5000)
			npcHandler.topic[cid] = 8
		elseif player:getStorageValue(9960) == 1 and player:getStorageValue(9961) < 1 then
			npcHandler:say({"Ah, welcome, welcome! I felt that one! The Dark Lord is pleased with the gift you brought, so now we can proceed with everything as planned. ...", "If it had gone wrong though, he would have had your guts for gart... er... well, here you are, so - ready to get some {blood} flowing?"}, cid, 0, 1, 5000)
			player:setStorageValue(9961, 1)
		elseif player:getStorageValue(9961) == 1 and player:getStorageValue(9962) < 1 then
			npcHandler:say({"Yes, I said blood. Important ingredient in necromantic rituals, usually. ...", "You need to spill some vials of blood tincture for this task. Probably means killing blood priests to get those vials. Ready to do this?"}, cid, 0, 1, 5000)
		...
		elseif player:getStorageValue(9965) == 1 and player:getStorageValue(9966) < 1 then
			...
			npcHandler:say({"Ah, it is invigorating to fell the dark flows, rushing through Drefia, once again! Capital. This has earned you a reward. ...", "Now, the next mission awaits, {yes}?"}, cid, 0, 1, 5000)
			npcHandler.topic[cid] = 9
		elseif player:getStorageValue(9969) == 1 and player:getStorageValue(9970) < 1 then
			npcHandler:say({"Ah, finally. Exceptional! Don't you feel the earth awakening to our call? No? Oh. ...", "Anyway, you succeeded in hallowing the Fireglass Altar! Only one altar remains to be hallowed! Shall we {proceed}?"}, cid, 0, 1, 5000)
		...
		elseif player:getStorageValue(9975) == 1 and player:getStorageValue(9976) < 1 then
			...
			npcHandler:say({"Yes! YES! I have felt that! The altars are alive again! Well done! ...", "We are close now. Only one important thing remains: the incantation itself. We need the {scroll} for that."}, cid, 0, 1, 5000)
		...
		elseif player:getStorageValue(9927) == 1 and player:getStorageValue(9928) < 1 then
			npcHandler:say({"Good! As you can imagine, I had a scroll piece, too. I hid it in my old quarters, northwest of the library. ...", "The door is magically sealed. Use this copper key with it to get inside. ...", "Beneath one of the chests is a secret stash, under a loose stone tile. The scroll piece should be inside. Off you go."}, cid, 0, 1, 5000)
		...
		elseif player:getStorageValue(9934) == 1 and player:getStorageValue(9935) < 1 then
			...
			npcHandler:say({"Hahah, eager for it, I like that! ...", "One piece stayed in the hands of a beautiful priestess. ...", "As they never throw anything away that looks like an incantation - you get my drift. Find out where they still keep it! ...", "Oh, one more thing! They only talk to you when you look like a fellow summoner. It's dangerous to go without a cape. Use this."}, cid, 0, 1, 5000)
		...
		elseif player:getStorageValue(9941) == 1 and player:getStorageValue(9942) < 1 then
			npcHandler:say({"That's the spirit! Speaking of which, you will have to look for a White Shade ...", "That's a ghost, in case you don't know. Goes by name of... Zarifan, if I recall correctly. ...", "His grave is somewhere to the south, somewhere deeper. And he only reacts to magic words. ...", "Err... it's embarrassing... the magic words are <mumble> <mumble> <ahem> ...", "'Friendship lives forever.' Silly, really. Untrue, as well. ...", "Anyway, say those three words - don't make me repeat them - and the old softie will tell you where the scroll is. Pathetic."}, cid, 0, 1, 5000)
			player:addItem(21464, 1)
		...
		elseif player:getStorageValue(9949) == 1 and player:getStorageValue(9876) < 1 then
			npcHandler:say({"It failed! IT FAILED! WHY? What have you done! This must be your fault! ...", "You... did... that was a recipe for chicken soup! No wonder the scroll failed! Now... all is lost ...", "<sobs drily> I never want to see you again! You and your dirty gravedigger hands! Take this, you ungrateful, useless, imbecile... human! ...", "Now... get out! And never, ever, dare come back! You ruined EVERYTHING!"}, cid, 0, 1, 5000)
	...
	elseif msgcontains(msg, "yes") then
		...
		elseif npcHandler.topic[cid] == 6 and player:getStorageValue(9952) == 1 then
			if player:removeItem(21403, 1) then
				...
				npcHandler:say({"Ah... interesting. A snake? Not bad, not bad at all. ...", "Right. Now, we need to waken the old powers through ritual. Let me know when you are ready for this {mission}."}, cid, 0, 1, 5000)
				npcHandler.topic[cid] = 0
			else
		...
		elseif npcHandler.topic[cid] == 9 and player:getStorageValue(9966) == 1 and player:getStorageValue(9967) < 1 then
			npcHandler:say({"Excellent. To bind the earthly powers, we, I mean you, must worship at the Fireglass Altar. ...", "This means scattering sacred ashes from the Ember Chamber on the sacrificial stones of the Fireglass Altar. ...", "To gather the sacred ashes, you have to use magic chalk with the Shadow Fire in the Ember Chamber. ...", "Gather the resulting ashes from the Shadow Hearth, and scatter them on the Fireglass Altar's sacrificial stones. Then return here."}, cid, 0, 1, 5000)
		...
		elseif npcHandler.topic[cid] == 13 and player:getStorageValue(9933) == 1 and player:getStorageValue(9934) < 1 then
			if player:removeItem(21250, 1) then
				...
				npcHandler:say({"Oh, praise the Dark Lord! It is my scroll piece! Give it here! You can have this instead. ...", "Ahh, that feels GOOD. Now, only two pieces left to hunt down! On to the {next} scroll piece, yes?"}, cid, 0, 1, 5000)
				npcHandler.topic[cid] = 0
			else
				npcHandler:say("You don't have my scroll.", cid)
				npcHandler.topic[cid] = 0
		...
		elseif npcHandler.topic[cid] == 14 and player:getStorageValue(9940) == 1 and player:getStorageValue(9941) < 1 then
			if player:removeItem(21250, 1) then
				...
				npcHandler:say({"Ahhh, capital, capital. Good girl for keeping it for me. I'll take back my cape now, thank you. ...", "Now - the final part of the scroll! Ready to go retrieve it?"}, cid, 0, 1, 5000)
			else
				npcHandler:say("You don't have my scroll.", cid)
				npcHandler.topic[cid] = 0
			end
		...
		elseif npcHandler.topic[cid] == 15 and player:getStorageValue(9944) == 1 and player:getStorageValue(9945) < 1 then
			if player:removeItem(21250, 1) then
				...
				npcHandler:say({"I am so excited! Finally, we - wait. What is this? That... is not the complete scroll piece. ...", "I don't care what you say! This is a disaster! We need a complete scr - wait. I have an idea. ...", "There must be a copy or something in my old library. something to use with the scroll, to find the missing words. ...", "There must be. Go look there. Next to the fiveserrated room, a small library. Go go go!"}, cid, 0, 1, 5000)
			else
				npcHandler:say("You don't have my scroll.", cid)
				npcHandler.topic[cid] = 0
			end
		elseif npcHandler.topic[cid] == 17 and player:getStorageValue(9947) == 1 and player:getStorageValue(9948) < 1 then
			npcHandler:say({"Then let's go. Take my skull and the incantation scroll to the working station in the fiveserrated room ...", "Where the lava flows and the southern legs of the room meet. Place the Skull on the sacrificial stone and use the scroll ...", "AND I shall be made whole, and YOU shall be RICH!"}, cid, 0, 1, 5000)
	...
	elseif msgcontains(msg, 'undertake') and player:getStorageValue(9953) == 1 and npcHandler.topic[cid] == 7  then
		npcHandler:say({"Good! Let's not waste time. The first altar you must hallow is the Dragonsoul Altar, at the eastern side of the room. ...", "To hallow it, dig out three dragon tears and place them on the altar to appease the dark powers. ...", "What? Oh, you'll find some dragon tears near dragon skulls... where? ...", "In the Dragonbone Cemetery, OBVIOUSLY! Must I explain EVERYTHING to you? Now go!"}, cid, 0, 1, 5000)
	...
	elseif msgcontains(msg, 'restore') and player:getStorageValue(9946) == 1 and player:getStorageValue(9947) < 1 and npcHandler.topic[cid] == 16 then
		if player:removeItem(21474, 1) then
			npcHandler:say({"I knew it! I knew I had made a copy! Oh, I am so clever! ...", "Now, watch this. <mumbles darkly> THERE. THE COMPLETE INCANTATION SCROLL. ...", "You have done well. Only one thing remains, and you shall be rich beyond your dreams. Ready for the really final task?"}, cid, 0, 1, 5000)
