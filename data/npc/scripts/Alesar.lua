...
local function greetCallback(cid, message)
	...
	if not msgcontains(message, 'DJANNI\'HAH') and player:getStorageValue(GreenDjinn.MissionEnd) < 3 or player:getStorageValue(Factions) <= 0 and not BlueOrGreen then
		return false
	end
	return true
end
