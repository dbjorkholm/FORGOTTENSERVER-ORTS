		...
		if player:getStorageValue(Storage.TheIceIslands.Questline) >= 21 then
			...
			elseif itemEx.itemid == 2733 then
				if player:getStorageValue(Storage.TheIceIslands.FlowerCactus) < 1 then
					...
					addEvent(function(toPosition) local tile = toPosition:getTile() if tile then local thing = tile:getTopVisibleThing() if thing and thing:isItem() then thing:transform(2733) end end end, 60 * 1000, toPosition)			
			...
			elseif itemEx.itemid == 4017 then
				if player:getStorageValue(Storage.TheIceIslands.FlowerBush) < 1 then
					...
					addEvent(function(toPosition) local tile = toPosition:getTile() if tile then local thing = tile:getTopVisibleThing() if thing and thing:isItem() then thing:transform(4017) end end end, 60 * 1000, toPosition)			
					toPosition:sendMagicEffect(CONST_ME_MAGIC_GREEN)
