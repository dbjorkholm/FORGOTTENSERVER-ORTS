	...
	for i = 1, #config do
		...
		if rand >= randomItem['from'] and rand <= randomItem['to'] then
			if randomItem['itemid'] then
				...
				doCreatureSayWithRadius(cid, "You found " .. (count > 1 and count or (itemType:getArticle() ~= "" and itemType:getArticle() or "")) .. " " .. (count > 1 and itemType:getPluralName() or itemType:getName()) .. " in the bag.", TALKTYPE_ORANGE_1, 1, 1)
			else
