...
function onSay(cid, words, param)
	...
	if(getPlayerStorageValue(cid, v.outfit.storage) < 1) then
		...
		if(removeItems == #v.items) then
			...
			doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Enjoy your new addons to your "..param.." outfit!")
                     doSendMagicEffect(getThingPos(cid), CONST_ME_FIREWORK_YELLOW)
			setPlayerStorageValue(cid, v.outfit.storage, 1)
		else
			for i = 1, #v.items do
				doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "You need " .. v.items[i][2] .. "x " .. getItemName(v.items[i][1]) .. " for the whole "..param.." addon.")
			end
		end
	else
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "You have already obtained addons to your "..param..".")
	end
	return false
end